...
def compute_merkle_root(leaves: list[str]) -> str:
    ...
    Raises AuditLogError if `leaves` is empty.
    """
    if not leaves:
        raise AuditLogError("cannot compute root of empty tree")
...
def build_merkle_proof(
    ...
    Raises AuditLogError if leaves is empty or index out of range.
    If `root` is provided, it is stored on the proof; otherwise it is
    re-computed from `leaves`.
    """
    if not leaves:
        raise AuditLogError("cannot build proof for empty tree")
    if index < 0 or index >= len(leaves):
        raise AuditLogError(f"index {index} out of range [0, {len(leaves) - 1}]")
